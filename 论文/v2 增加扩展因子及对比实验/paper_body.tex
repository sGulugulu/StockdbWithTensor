...
进一步基于全 A 样本的 Tucker 选择结果生成了股票潜在结构图与 cluster-vs-industry 交叉图。对应产物位于 \texttt{code/data/formal/reports/pattern\_discovery/}，其中 \texttt{stock\_latent\_structure\_formal\_all\_a\_run\_tucker.svg} 将股票按潜在聚类标签展开，并用行业颜色展示聚类内部的行业构成；\texttt{cluster\_vs\_industry\_formal\_all\_a\_run\_tucker.svg} 则把聚类标签与行业分布交叉统计。该图组说明，股票潜在空间并不是简单复制行业分类，而是在行业信息之外保留了由因子暴露、时间状态和排序得分共同形成的结构相似性。
...
\nextgroupplot[
  ...
  symbolic y coords={SIZE\_LARGE,SIZE\_SMALL,INDUSTRY\_C39,INDUSTRY\_C35,SIZE\_MID,ZZ500,ALL\_A,HS300,SZ50,INDUSTRY\_C27},
  ytick=data,
  xlabel={Rank IC}
]
...
(-0.2297,SIZE\_LARGE)
(-0.2325,SIZE\_SMALL)
(-0.0579,INDUSTRY\_C39)
(0.0434,INDUSTRY\_C35)
(0.0524,SIZE\_MID)
(0.0573,ZZ500)
(0.0598,ALL\_A)
(0.0731,HS300)
(0.1523,SZ50)
(0.2364,INDUSTRY\_C27)
};
\nextgroupplot[
  title={Tucker跨边界累计收益},
  symbolic y coords={SIZE\_LARGE,SIZE\_SMALL,INDUSTRY\_C39,SIZE\_MID,ALL\_A,ZZ500,SZ50,INDUSTRY\_C35,HS300,INDUSTRY\_C27},
  ytick=data,
  xlabel={累计收益(\%)}
]
...
时间模态则提供了对市场状态切换的观察窗口。正式实验输出的 \texttt{time\_regimes\_tucker.json} 记录了若干显著的相邻日期结构跳变。在当前已提交的 HS300 正式结果中，最明显的跳变集中在 2026 年 3 月 25 日至 2026 年 3 月 30 日这一短区间，说明即使在较短的正式输出窗口内，潜在结构也并非静态稳定，而是在局部阶段出现明显重排。换言之，张量分解不仅用于“压缩因子”，还能够识别“市场何时发生了潜在结构变化”。
...
为了验证扩展训练输入是否能够进入统一训练接口，本文进一步构造了 extended factor panel，并在 HS300、SZ50 与 ZZ500 三个样本池上分别运行 baseline/extended 两套 formal 配置。扩展版在原有四个主线因子之外，新加入市场级代理变量 \texttt{market\_return\_1d}、\texttt{market\_momentum\_5d}、\texttt{market\_momentum\_20d}、\texttt{market\_volatility\_20d}、\texttt{market\_drawdown\_20d}、\texttt{market\_amount\_change\_5d}、\texttt{market\_amount\_zscore\_20d}，外部宏观变量 \texttt{macro\_policy\_rate}、\texttt{macro\_lpr\_1y}、\texttt{macro\_lpr\_5y}、\texttt{macro\_cpi\_mom}、\texttt{macro\_m2\_yoy}、\texttt{macro\_industrial\_production\_yoy}、\texttt{macro\_exports\_yoy}、\texttt{macro\_imports\_yoy}，财务 PIT 变量 \texttt{pit\_roe\_avg}、\texttt{pit\_np\_margin}、\texttt{pit\_gp\_margin}、\texttt{pit\_eps\_ttm}，分红变量 \texttt{dividend\_*}，以及业绩快报/业绩预告与公告事件相关的 \texttt{perf\_express\_*}、\texttt{forecast\_*}、\texttt{major\_event\_*}、\texttt{announcement\_*} 系列特征。对照结果表明，扩展输入的影响并不呈现单边改善，而是具有明显的样本池差异。
...
为了进一步检验结论是否依赖于单一指数样本池，本文在当前提交版仓库中补充了样本边界扩展实验，包括全 A 活跃股票样本、三个行业分层样本以及大中小市值分层样本。与 HS300、SZ50、ZZ500 的正式基线相比，这批扩展实验更强调“不同样本边界下模型行为是否稳定”，首先在当前提交版短窗口口径下完成第一批 run，并进一步基于 long-window 因子面板对 2015--2026 全部年度做年度复核，重点观察 Rank IC、滚动稳定性和组合累计收益的相对变化。
...
在全年 long-window 复核中，这种“最优模型随边界和年份切换”的现象依然存在。根据 \texttt{code/data/formal/reports/long\_window\_portfolio/README.md}，全 A 样本在 2015 窗口中呈现“Rank IC 最优为 CP、收益最优为 PCA”的结构；2019 年三类模型的累计收益全部转正，但最优仍由 PCA 占据；2021 年则出现 Tucker 同时在 Rank IC、收益与 Sharpe 上占优的窗口；2026 年的短年度窗口又重新回到“PCA 在 Rank IC 上领先、CP 在收益上略占优”的局面。行业与市值分层同样没有表现出跨年份恒定不变的最优模型，例如 2021--2023 年的行业 C27 与 C39 更频繁出现 Tucker 最优，而 2018、2019、2025 等窗口里 PCA 或 CP 也会重新占优。总体而言，这些新增 run 已经把“样本边界扩展”从计划推进为真实产物，并证明结论不仅依赖样本边界，也依赖市场阶段。
...
\nextgroupplot[
  ...
  symbolic y coords={INDUSTRY\_C39,ALL\_A,SIZE\_SMALL,INDUSTRY\_C35,INDUSTRY\_C27,SIZE\_MID,SIZE\_LARGE},
  ytick=data,
  xlabel={Rank IC}
]
\addplot[fill=teal!65] coordinates {
(-0.1234,INDUSTRY\_C39)
(-0.1010,ALL\_A)
(-0.0903,SIZE\_SMALL)
(-0.0316,INDUSTRY\_C35)
(0.0038,INDUSTRY\_C27)
(0.0553,SIZE\_MID)
(0.1097,SIZE\_LARGE)
};
\nextgroupplot[
  title={2026 long-window Tucker 期末净值},
  symbolic y coords={INDUSTRY\_C39,SIZE\_SMALL,SIZE\_MID,INDUSTRY\_C35,ALL\_A,SIZE\_LARGE,INDUSTRY\_C27},
  ytick=data,
  xlabel={期末净值}
]
\addplot[fill=purple!45] coordinates {
(0.7241,INDUSTRY\_C39)
(0.7473,SIZE\_SMALL)
(0.7883,SIZE\_MID)
(0.8270,INDUSTRY\_C35)
(0.8339,ALL\_A)
(0.8694,SIZE\_LARGE)
(1.2030,INDUSTRY\_C27)
};
\end{groupplot}
\end{tikzpicture}
...
当然，本文仍存在若干需要继续完善的方面。第一，在数据输入层面，当前正式训练输入已经在 extended 配置中接入市场状态代理、外部利率、宏观月度指标、财务 PIT、分红以及公告事件字典特征，并把字段字典、披露日和最早可用交易日规则写入扩展合同。但当前 committed snapshot 的外部宏观与公告文本仍以当前实验窗口为主，尚未扩展到更长历史与更完整正文全文，这意味着当前模型虽然已经验证了“更宽输入边界可被 PIT-safe 接入”，但对于更宽口径、多来源因子体系的长期适应性仍需在源表持续扩充后继续检验。
...
第三，在样本边界层面，本文已经从 HS300、SZ50 与 ZZ500 三个指数样本池扩展到全 A 活跃股票、行业分层和市值分层实验，并为 2015--2026 年区间完成了按年度拆分的 long-window 真实复跑。新增结果说明，指数样本上的结论不能被机械外推：Tucker 在部分窗口和部分行业样本中表现较稳健，但电子、专用设备和不同市值分层样本的最优模型并不固定。虽然全年 long-window 结果已经补齐，但当前训练输入仍是“主线四因子 + 市场代理 + 外部宏观 + 财务 PIT + 分红 + 公告事件字典”的 committed snapshot，这批扩展实验更适合被理解为“当前可审计输入边界在不同边界与市场阶段下的稳健性检验”，而不是所有候选输入都已完备后的最终全市场结论。

第四，在可视化与论证层面，本文已经补充解释方差、Rank IC、滚动稳定性、时间状态、因子热力图、股票潜在结构图、行业聚类交叉图和跨样本边界对比图，并将这些图表整理为答辩材料素材包，能够比单纯模型比较更完整地支撑模式发现叙事。进一步补充的 2015--2026 各年度全 A 长窗口时间状态切换图、因子重要性热力图，以及按年度组织的 \texttt{code/data/formal/reports/pattern\_discovery\_long\_window\_<year>/} 图组，已经把此前“长窗口图待补”的占位说明替换为真实证据。当前仍需继续完善的是把这些 long-window 图组与更宽输入边界实验联动，而不是继续停留在主线四因子输入上。
...
不足之处主要包括：虽然全 A、行业和市值分层实验已经补充到 2015--2026 全部 long-window 年度，并形成了 \texttt{long\_window\_portfolio} 汇总表与按年度组织的 long-window 模式发现图组，同时当前 committed snapshot 已经把外部宏观、分红、重大事项和公告标题文本纳入训练接口，但这些源表仍以当前实验窗口为主要服务对象，尚未扩展到更长历史、更完整宏观品种和公告正文全文；当前实验已经形成 Top-N 组合净值、分位数组、多空、成本后净值、基准超额收益和跨样本边界统一汇总，但交易冲击和风险归因仍有继续细化空间；模式发现图表已经从基础指标扩展到股票潜在结构、行业聚类关系和全年 long-window 图组，但这些结果仍建立在当前 committed input snapshot 之内。参考文献元数据已经按当前学校提交口径完成最终校验，后续只有在学院模板明确要求逐条展示 DOI 时才需要统一追加。
