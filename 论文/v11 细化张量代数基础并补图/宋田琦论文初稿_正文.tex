...
模态展开又称张量矩阵化，是指固定某一个模态作为矩阵的行，而将其余模态按照约定顺序展开为列。对三阶张量$\mathcal{X}\in\mathbb{R}^{N\times F\times T}$而言，若以股票模态为行，则得到第一模展开矩阵$X_{(1)}\in\mathbb{R}^{N\times (FT)}$；若以因子模态为行，则得到第二模展开矩阵$X_{(2)}\in\mathbb{R}^{F\times (NT)}$；若以时间模态为行，则得到第三模展开矩阵$X_{(3)}\in\mathbb{R}^{T\times (NF)}$。其本质是把一个三维数组在不同观察角度下改写为矩阵，从而把原本的高阶优化问题转化为矩阵上的计算问题。
...
图\ref{fig:mode1-unfolding}给出了第一模展开的具体过程。可以看到，第一模展开并不是改变张量中元素的数值，而是将不同时间切片中的因子列依次排列到同一个矩阵中，使每一行仍然对应同一只股票。这样处理以后，股票模态的信息被保留在矩阵行方向，而因子模态与时间模态被合并到列方向，后续算法便可以在矩阵形式下估计股票侧的潜在载荷。

与第一模展开类似，第二模和第三模展开只是更换“作为行的模态”。当以因子模态为行时，矩阵的每一行对应一个因子在不同股票和时间下的取值组合，更适合观察因子之间的共同变化；当以时间模态为行时，矩阵的每一行对应一个观测时点，更适合分析市场状态随时间变化的结构。因此，三种展开方式本质上对应三种不同研究视角，而不是三组互不相关的数据变换。
...
对本文而言，Khatri--Rao 积最重要的作用是把因子模态和时间模态的信息组合成一个统一矩阵，以便与某一模态展开矩阵一起参与最小二乘更新。换句话说，它是 CP 分解中“跨模态耦合信息合并”的核心工具。
...
在本文后续的 CP 更新公式中，Hadamard 积出现在 Gram 矩阵的组合项中，即用来整合不同模态因子矩阵之间的内积信息。虽然形式简单，但它在求解过程中承担了把多个模态的二阶信息合并起来的作用。
...
上述几类运算并不是彼此孤立的符号堆叠，而是在后文张量分解模型中共同发挥作用。模态展开把三阶张量转化为矩阵形式，为交替最小二乘提供运算入口；外积给出了秩一张量的构造方式，是 CP 分解最基本的组成单元；Kronecker 积是理解列向量耦合扩展的基础；Khatri--Rao 积则进一步把两个模态的列信息逐列组合起来；Hadamard 积用于整合不同模态 Gram 矩阵之间的按元素结构信息。若把这些运算联系起来看，就可以更容易理解后文中的更新公式为何同时出现$X_{(1)}$、$\odot$与$\ast$等符号。
...
将本文结果与前述文献放在同一视角下观察，可以更清楚地说明本研究的补充意义。黄宏运与刘宇轩的研究表明，多因子模型即使在传统回归框架下，也能够通过因子筛选与宏观周期修正获得可观的选股效果\textsuperscript{[13][14]}；本文的结果并不否认这一点，而是进一步说明，当研究对象由“少量已筛选因子”扩展到更高维的因子集合时，问题的核心会从“因子是否有效”转向“高维信息如何被稳定压缩并用于排序”\textsuperscript{[15][16]}。与此同时，岳欣与曾亚丽关于张量化金融数据的研究强调了时间结构和高阶表示的重要性\textsuperscript{[8][10]}，而本文在样本边界扩展、长窗口年度复核以及时间状态切换结果中的发现，则进一步表明这种高阶结构优势不仅体现在波动分析或时序预测上，也可以体现在股票因子排序与模式识别任务中\textsuperscript{[9][11][12][17]}。换言之，本文希望在多因子研究与张量表示研究之间建立更直接的实证联系。
