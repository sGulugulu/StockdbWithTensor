...
class ExtendedFactorPanelTests(unittest.TestCase):
    def test_build_extended_factor_panel_merges_pit_and_event_features(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(float(row2["forecast_age_days"]), 0.0)
            self.assertEqual(float(row2["perf_express_age_days"]), 0.0)
            self.assertEqual(float(row2["pit_data_age_days"]), 5.0)
            ...
            self.assertEqual(float(row2["major_event_age_days"]), 5.0)
