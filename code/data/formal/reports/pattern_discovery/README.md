...
| 时间状态切换 | 各 formal run 的 `time_regime_timeline.svg` | 用于解释短窗口内市场状态变化；长窗口版本可按 `code/data/formal/reports/long_window_plan/README.md` 中的派生配置逐年替换 |
| 因子重要性 | 各 formal run 的 `factor_importance_heatmap.svg` | 用于说明价值、动量、质量、波动率及扩展输入在低维结构中的贡献 |

面向最终答辩时，优先使用 `code/data/formal/reports/defense_materials/README.md` 中的素材包清单，该清单把本目录图组与组合闭环报告、extended 输入合同放在同一条讲述路径下。
